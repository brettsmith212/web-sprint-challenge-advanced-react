...
Class-based components handle state mangement differently. You will set the state in the constructor of the class, rather than using useState() for each slice of state in functional components. Class based components also pass props using this.props whereas functional components pass them with just props. For the lifecycle methods, class based components use componentDidMount() whereas functional components use useEffect() for the same purpose.

2. When does a componentWillMount function be called? What about a componentDidUpdate?

componentWillMount is called at the very end of the react lifecycle. componentDidUpdate is called after state is changed in the react component, causing it to run and the component to be rerendered.

3. Define stateful logic.

Stateful logic is code that is dependent on data and how that data changes over time. The benefits of stateful logic is that it allows your program to make changes when the datas state is changed.

4. What are the three step of creating a successful test? What is done in each phase?

Arrange, Act, Assert. In Arrange you will organize the your input information and render the react element into the DOM. In Act you will use userEvents to make changes / click buttons for simulating user effects. In Assert you check for the expected results of your user effects.
